...
void LogisticRegression<OptimizerType>::Predict(const arma::mat& predictors,
    ...
    const
{
  for(int i = 0; i < predictors.n_rows; i++){
  arma::vec dummy = -(parameters(0) + predictors.col(i).t() * parameters.subvec(1, parameters.n_elem - 1));
  dummy.print();
  }
  ...
  responses = arma::pow( (0.5 / (1.0 + arma::exp(-parameters(0) - predictors.t() * parameters.subvec(1, parameters.n_elem - 1)))),1.0);
}
